...
namespace Spinnaker_Emu
{
    class NodeMemory
    {
        ...
        public void Clear()
        {
            uint Length = (uint)this.Memory.Length;
            this.AvailableBytes = Length;
            this.MemoryMap.Clear();
            this.FreeMemoryMap.Clear();
            this.FreeMemoryMap.Add(0, Length);
        ...
        public bool Delete(uint Address)
        {
            ...
            uint Len = (uint)MemoryMap[Address];

            this.AvailableBytes = this.AvailableBytes + Len;

            // Add to free memory map
            this.FreeMemoryMap.Add(Address, Len);

            // Defragment Memory Map
            for (uint i = 0; i < (this.FreeMemoryMap.Keys.Count - 1); i++)
            {
                uint Addr1 = (uint)this.FreeMemoryMap.GetKey((int)i);
                uint Len1 = (uint)this.FreeMemoryMap.GetByIndex((int)i);

                uint Addr2 = (uint)this.FreeMemoryMap.GetKey((int)i+1);
                uint Len2 = (uint)this.FreeMemoryMap.GetByIndex((int)i);

                if ((Addr1 + Len1) == Addr2) // Blocks are contiguous
                {
                    this.FreeMemoryMap.Remove(Addr1);
                    this.FreeMemoryMap.Remove(Addr2);

                    this.FreeMemoryMap.Add(Addr1, (Len1 + Len2));
                }
            }
